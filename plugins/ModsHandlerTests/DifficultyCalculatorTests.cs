...
{
    private readonly Dictionary<Mods, List<float>> OD_VALUES = new()
    {
    ...
    {
        DifficultyCalculator diffCalc = new();
        Beatmap beatmap = new();

        List<float> resultValues = new();
        for (int i = 10; i >= 0; i--)
        {
            beatmap.OverallDifficulty = i;
            float odResult = diffCalc.ApplyMods(beatmap, mods)["OD"];
