...
#define T_TEXT                          T_("SC output pattern name")
...
public:
	...
	MappingFile(string map_name)
	{
		...
		handle = CreateFileMappingA(INVALID_HANDLE_VALUE, NULL,
					    PAGE_READWRITE, 0, 8 * 1024,
					    map_name.c_str());
		//handle = OpenFileMappingA(FILE_MAP_READ, false, map_name.c_str());
		data = (TCHAR *)MapViewOfFile(handle, FILE_MAP_READ, 0, 0,
					      8 * 1024);
	}
...
void TextSource::LoadMappingFileText()
{
	if (mapping_file != NULL) {
		text = std::wstring(mapping_file->ReadToEnd());
	} else {
		text = L"Could not find StreamCompanion output pattern with specified name.";
	}
...
inline void TextSource::Update(obs_data_t *s)
{
	...
	if (mapping_file != NULL) {
		if (map_name != mapping_file->name)
			mapping_file = unique_ptr<MappingFile>(
				new MappingFile(map_name));
	} else {
		if (map_name != "") {
			mapping_file = unique_ptr<MappingFile>(
				new MappingFile(map_name));
...
OBS_DECLARE_MODULE()
OBS_MODULE_USE_DEFAULT_LOCALE("obs-text-sc", "en-US")
MODULE_EXPORT const char *obs_module_description(void)
{
	return "SC Windows GDI+ text source";
}
...
bool obs_module_load(void)
{
	...
	si.get_name = [](void *) { return obs_module_text("Text (GDI+) SC"); };
